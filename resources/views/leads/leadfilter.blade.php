...
@section('main')
    <ul class="justify-content-center mb-3 shadow p-1">
        <a class="link-offset-2 link-underline link-opacity-75-hover @if ($route == 'all') link-underline-opacity-100 @else link-underline-opacity-0 @endif"
            href="/filterleads/all">Все<span
                class="badge m-1 @if ($allleadscount == '0') text-bg-danger @else text-bg-secondary @endif">{{ $allleadscount }}</span></a>

        <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'new') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
            href="/filterleads/new">Новые<span
                class="badge m-1 @if ($newleadscount == '0') text-bg-danger @else text-bg-secondary @endif">{{ $newleadscount }}</span></a>

        <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'phone') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
            href="/filterleads/phone">Дозвон<span
        ...
        @if (auth()->user()->role != 'lead_handler')
            <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'consleads') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
                href="/filterleads/consleads">Консультация<span
                    class="badge m-1 @if ($consleads == '0') text-bg-danger @else text-bg-secondary @endif">{{ $consleads }}</span></a>

            <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'defeatleads') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
                href="/filterleads/defeatleads">Мусорные<span
                    class="badge m-1 @if ($defeatleads == '0') text-bg-danger @else text-bg-secondary @endif">{{ $defeatleads }}</span></a>

            <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'withoutcaseleads') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
                href="/filterleads/withoutcaseleads">Бездельники<span
                    class="badge m-1 @if ($withoutcaseleads == '0') text-bg-danger @else text-bg-secondary @endif">{{ $withoutcaseleads }}</span></a>

            <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'winleads') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
                href="/filterleads/winleads">Выигранные<span
                    class="badge m-1 @if ($winleads == '0') text-bg-danger @else text-bg-secondary @endif">{{ $winleads }}</span></a>

            <a class="link-offset-2 link-underline link-opacity-75-hover  @if ($route == 'failleads') link-underline-opacity-100 @else  link-underline-opacity-0 @endif"
                href="/filterleads/failleads">Проваленные<span
    ...
    <form class="row my-2" action="{{ route('leads.filter', ['leadfilter' => $route]) }}" method="get" autocomplete="off">
        ...
        <div class="row">
            <div class="col-md-2 col-12">
                ...
                <input type="text" class="form-control" value="{{ session('number') }}" name="findNumber"
                    id="findNumber" placeholder="телефон">
            </div>
            <div class="col-md-2 col-12">
                <label for="findName"><small>поиск по имени</small></label>
                <input type="text" class="form-control" value="{{ session('name') }}" name="findName" id="findName"
                    placeholder="ФИО (часть)">
            </div>

            <div class="col-md-2 col-12">
                <label for="datetimepicker3"><small>начало:</small></label>
                <input type="text" id="datetimepicker3" class="form-control" name="startdate"
                    value="{{ request()->input('startdate') }}">
            </div>

            <div class="col-md-2 col-12">
                <label for="datetimepicker4"><small>конец:</small></label>
                <input id="datetimepicker4" class="form-control" name="enddate" type="text"
            ...
            <div class="col-md-2 col-12">
                ...
                <a type="reset" href="{{ route('leads.filter', ['leadfilter' => $route]) }}"
                    class="btn btn-secondary form-control">сбросить</a>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-2 col-12">
                ...
                <a type="button" href="https://t.me/nedicomlead" class="btn btn-light form-control" target="_blank">В
                    группу</a>
            </div>
    ...
    <script>
        ...
        window.onload = (event) => {
            let elementsArray = document.getElementsByClassName("leadconsstatus");

            Array.prototype.forEach.call(elementsArray, function(el) {
                el.addEventListener("change", function() {
                    $.ajax({
                        url: '/leads/change/' + $(this).find(':selected').data('id'),
                        type: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: {
                            'leadconsstatus': el.value
                        }
