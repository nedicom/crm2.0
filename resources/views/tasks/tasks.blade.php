        ...
        <div class="col-12">
            <div class="row">
                <div class="col-12 col-md-2 my-2">
                    <a href="{{ route('tasks') }}?checkedlawyer={{ Auth::user()->id }}"
                        class="btn btn-outline-primary btn-sm">мои задачи</a>
                </div>
    ...
    <div class="row" id="taskarea">
        ...
        @if (
            $request->input('calendar') == '' ||
                $request->input('calendar') == \App\Models\Enums\Tasks\DateInterval::AllTime->name ||
                \App\Helpers\TaskHelper::isDayInterval($request))
            <div class="row pt-4">
                <div class="col-md-3 columncard text-center" id="timeleft">
                    <h5 class="page-title" data-bs-toggle="tooltip" data-bs-placement="top"
                        data-bs-title="Сюда попадают просроченные задачи каждый день в 8.00 утра">просрочка</h5>
                    @foreach ($data as $el)
                ...
                <div class="col-md-3 columncard text-center" id="waiting">
                    <h5 class="page-title" data-bs-toggle="tooltip" data-bs-placement="top"
                        data-bs-title="Тут задачи которые Вам поставили, но не принятые в работу">ожидает</h5>
                    @foreach ($data as $el)
                ...
                <div class="col-md-3 columncard text-center" id="inwork">
                    <h5 class="page-title" data-bs-toggle="tooltip" data-bs-placement="top"
                        data-bs-title="Здесь задачи с которыми вы работаете сейчас. Позже будет учитываться время потраченное на выполнение">
                        в работе</h5>
                    @foreach ($data as $el)
                ...
                <div class="col-md-3 columncard text-center" id="finished">
                    <h5 class="page-title" data-bs-toggle="tooltip" data-bs-placement="top"
                        data-bs-title="Каждый день в 00.00 выполненные задачи будут пропадать из списка">выполнена</h5>
        ...
        @if ($request->input('calendar') == \App\Models\Enums\Tasks\DateInterval::Week->name)
            <div class="mt-2" style="display: grid; grid-template-columns: 20% 20% 20% 20% 20%;">
                ...
                @endfor
                @for ($i = 1; $i < 6; $i++)
                    <div class="columncard bg-white m-1" style="font-size:12px; min-height: 400px"
                        dayofweek="{{ $i }}">
        ...
        @if ($request->input('calendar') == \App\Models\Enums\Tasks\DateInterval::Day->name)
            ...
            @for ($i = 8; $i < 22; $i++)
                <div class='row'>
                    ...
                    <div style="min-height: 200px;" class="col-11  bg-white my-3 columncard"
                        hourofday="{{ $i }}">
                        @foreach ($data as $el)
                ...
                $(function() {
                    $(".columncard").sortable({
                        ...
                        receive: function(event, ui) {
                            ...
                            $input = $(this);
                            if ($input.attr("dayofweek")) {
                                var dayofweek = $input.attr("dayofweek");
                            } else {
                                var dayofweek = 0;
                            }
                            if (ui.item.attr("date")) {
                                var date = ui.item.attr("date");
                            } else {
                                var date = 0;
                            }
                            if ($input.attr("hourofday")) {
                                var hourofday = $input.attr("hourofday");
                            } else {
                                var hourofday = 0;
                            }
                            if ($input.attr("dayofmonth")) {
                                var dayofmonth = $input.attr("dayofmonth");
                            } else {
                                var dayofmonth = 0;
                            }
