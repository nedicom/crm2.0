                        ...
                        <form action="{{ route('adddogovor') }}" autocomplete="off" method="post">
                                ...
                                <div class ="collapse" id="collapseService">
                                    @foreach ($dataservice as $el)
                                        ...
                                        <span onclick="addServiceFunc(this)" data-price = "{{ $el->price }}"
                                            class="badge bg-{{ $input[$rand_key] }} shadow m-1">{{ $el->name }}
                                            ({{ $el->price }})</span>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="form-group mb-3 col-9">
                                    <label for="subject">Укажите предмет (услуги)<span
                                            class="text-danger">*</span></label>
                                    <textarea rows="5" name="subject" value="{{ old('subject') }}"
                                        placeholder="Исковое заявление о признании права собственности, участие в судебном заседании..." id="subject"
                                        class="form-control" required></textarea>
                                </div>

                                <div class="form-group mb-3 col-3">
                                    <label for="allstoimost">Cтоимость <span class="text-danger">*</span></label>
                                    <input type = "text" name="allstoimost" placeholder=""
                                        value="{{ old('allstoimost') }}" id="allstoimost" class="form-control" required>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="form-group mb-3 col-9">
                                    <label for="preduslugi">Укажите какие услуги предоплачены<span
                                            class="text-danger">*</span></label>
                                    <textarea rows="3" name="preduslugi" value="{{ old('preduslugi') }}"
                                        placeholder="Например, исковое заявление о признании права собственности." id="preduslugi" class="form-control"
                                        required></textarea>
                                </div>
                                <div class="form-group mb-3 col-3">
                                    <label for="predoplata">Предоплата<span class="text-danger">*</span></label>
                                    <input type = "text" name="predoplata" value="{{ old('predoplata') }}"
                                        placeholder="" id="predoplata" class="form-control" required>
                                </div>
                            </div>

                            <div class="row">
                                <div class="form-group mb-3 col-5">
                                    <label for="client">Укажите клиента (<a href="clients" target="_blank">создать
                                            нового</a>)<span class="text-danger">*</span></label>
                                    <input type = "text" name="client" value="{{ old('client') }}" id="client"
                                        class="form-control" required>
                                    <div id="clientList">
                                    </div>
                                    <div id="client" class="form-text">Вы не можете создавать нового клиента, но
                                        можете изменить имя клиента, если в нем ошибка</div>
                                </div>

                                <div class="form-group mb-3 col-5">
                                    <label for="adress">Укажите адрес клиента<span
                                            class="text-danger">*</span></label>
                                    <input type = "text" name="adress" value="{{ old('adress') }}" placeholder=""
                                        id="adress" class="form-control" required>
                            ...
                            <div class="d-flex align-items-center">
                                <p><button type="submit" id='submit' class="btn btn-primary"
                                        disabled>Сохранить</button></p>
                                <p class="small mx-3" id="newclient" style="color:red;">Нового клиента добавлять
                                    нельзя</p>
                            </div>
