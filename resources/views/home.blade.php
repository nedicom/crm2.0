    ...
    <form action="{{ route('home') }}" class="my-3" method="GET" id="form-filter">
        <div class="d-inline-flex flex-column px-3 m-1 mb-3">
            <div class="form-check">
                <input class="btn-check input-home-filter" type="radio" name="date" id="day" value="day"
                    @if ('day' == request()->get('date')) checked @endif>
                <label class="btn btn-light" for="day">День</label>
                <input class="btn-check input-home-filter" type="radio" name="date" id="month" value="month"
                    @if ('month' == request()->get('date')) checked @endif>
        ...
        <div class="px-3 m-1">
            <select class="form-select form-select-sm input-home-filter" name="lawyer">
                ...
                @foreach ($data['datalawyers'] as $el)
                    <option value="{{ $el->id }}" @if ($el->id == request()->get('lawyer')) selected @endif>
                        {{ $el->name }}</option>
                @endforeach
            </select>
        </div>
...
@section('main')
    <div class="row">
        ...
        <div class="col-md-6 col-12">
            ...
            <div class="mb-2">
                @if ($user->tg_id)
                    <a href="{{ config('app.bot_staff.link') }}?start={{ base64_encode($user->id) }}">Бот-информер
                        подключен</a>.
                @else
                    Для подключения бота-инфомера к своему аккаунту
                    <a href="{{ config('app.bot_staff.link') }}?start={{ base64_encode($user->id) }}">перейдите по
                        ссылке</a>.
                @endif
            </div>
            ...
            @if (Auth::user()?->role != 'user')
                ...
                <div class="pt-2">
                    ...
                    <form enctype="multipart/form-data" action="{{ route('yandexmap') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <input class="form-control" type="file" id="yandexcsv" name="yandexcsv" accept=".csv"
                                    required>
                            </div>
                            <div class="col-3">
    ...
    <div class="row mt-5">
        <h3>Показатели <small class="text-muted">
        ...
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill" data-bs-target="#pills-home"
                    type="button" role="tab" aria-controls="pills-home" aria-selected="true">Исполнитель</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-contact-tab" data-bs-toggle="pill" data-bs-target="#pills-contact"
                    type="button" role="tab" aria-controls="pills-contact"
                    aria-selected="false">Постановщик</button>
            </li>
        </ul>
                ...
                @foreach ($all['alltasksnew'] as $el)
                    <div class="d-flex text-body-secondary pt-3">
                        <svg class="bd-placeholder-img flex-shrink-0 me-2 rounded" width="32" height="32"
                            ...
                            <rect width="100%" height="100%" fill="#ff0000"></rect><text x="50%" y="50%"
                                fill="#ff0000" dy=".3em">{{ $el->type }}</text>
                        </svg>
                        <div class="pb-3 mb-0 small lh-sm border-bottom w-100">
                            <div class="d-flex justify-content-between">
                                ...
                                <span
                                    @if ($el->tag == 'неважно') class="badge bg-secondary rounded-pill"
                                    ...
                                    @else
                                    class="badge bg-secondary rounded-pill" @endif>
                                    <div>{{ $el->tag }}</div>
                                </span>
                            </div>

                            <a href="tasks/{{ $el->id }}" class="text-decoration-none"
        ...
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="pills-isp-tab" data-bs-toggle="pill" data-bs-target="#pills-isp"
                    type="button" role="tab" aria-controls="pills-isp" aria-selected="true">Исполнитель</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="pills-post-tab" data-bs-toggle="pill" data-bs-target="#pills-post"
                    type="button" role="tab" aria-controls="pills-post" aria-selected="false">Постановщик</button>
            </li>
        </ul>
